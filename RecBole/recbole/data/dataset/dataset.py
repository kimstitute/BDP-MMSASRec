            ...
            if field in dis_info:
                ...
                for feat in self.field2feats(field):
                    ...
                    if ftype == FeatureType.FLOAT:
                        ...
                        res, self.field2bucketnum[field] = disc(
                            np.concatenate(seq_values), method, bucket
                        )
                        ret = np.ones_like(res)
                        res = np.split(res, split_point)
                        ret = np.split(ret, split_point)
                        feat[field] = pd.Series(list(zip(ret, res)), index=feat.index)
            else:
                for feat in self.field2feats(field):
                    ftype = self.field2type[field]
                    if ftype == FeatureType.FLOAT:
                        ...
                        seq_values = [np.asarray(seq) for seq in feat[field].tolist()]
                        feat[field] = pd.Series(
                            [(seq, np.ones_like(seq)) for seq in seq_values],
                            index=feat.index,
                        )
