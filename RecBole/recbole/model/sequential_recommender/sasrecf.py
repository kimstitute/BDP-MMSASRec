...
class SASRecF(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.feature_embed_fields = []
        self.direct_float_seq_fields = []
        for field in self.selected_features:
            if dataset.field2type[field] == FeatureType.FLOAT_SEQ:
                self.direct_float_seq_fields.append(field)
            else:
                self.feature_embed_fields.append(field)
        self.num_feature_field = len(self.feature_embed_fields) + len(
            self.direct_float_seq_fields
        )
        ...
        self.float_seq_projector = nn.ModuleDict(
            {
                field: nn.Linear(dataset.field2seqlen[field], self.hidden_size)
                for field in self.direct_float_seq_fields
            }
        )
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        for field in self.direct_float_seq_fields:
            feature = self.item_feat[field][item_seq]
            if feature.dim() == item_seq.dim() + 2 and feature.size(-1) == 2:
                feature = feature[..., 0]
            projected_feature = self.float_seq_projector[field](feature.float())
            feature_table.append(projected_feature.unsqueeze(-2))
